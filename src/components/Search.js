...
const Search = ({ onFormSubmit }) => {
  ...
  return (
    <form className="bg-white flex px-2 mt-2" onSubmit={onSubmit}>
      <input onChange={event => setName(event.target.value)} value={name} className="text-black w-full rounded my-2 px-3 focus:outline-none" type="text" placeholder="Enter summoner name" />
